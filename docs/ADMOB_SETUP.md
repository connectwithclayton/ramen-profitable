...
`npm test` exercises the application root and Store React tree, Zustand state, and public RevenueCat purchase/restore APIs, with native/network doubles. Current source coverage includes unknown and suppressed ownership, stale CustomerInfo ordering, fresh-purchase and exactly-once returning-owner earnings across production hydration order, lifecycle-clock isolation, consent deduplication and visibility-return recovery, non-personalized requests, notification/overlay/destination obstruction, full containment above the measured dock-fade boundary, adaptive widths, no-fill, and expiry recovery. The test files own the exact case inventory. Config coverage includes platform-neutral release configuration without iOS IDs, RevenueCat mode propagation to Expo Constants' later consumer, and the native identifier guard with missing, sample, cross-publisher, login-shell-drifted, and valid IDs; ads coverage executes the valid release JavaScript path. These tests are not proof of real ad fill, native iPad modes or creative rendering, App Store purchases, or dashboard setup.
