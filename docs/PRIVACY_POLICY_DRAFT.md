...
- [`src/components/PhoneBillboard.tsx`](../src/components/PhoneBillboard.tsx) mounts the native inline adaptive banner at its measured container width only when eligible, consent-ready, foregrounded, and unobscured by a game overlay. Every banner request is non-personalized and supplies no RevenueCat identity or custom targeting.
