...
**Test IDs cannot silently ship:** Expo config rejects missing, malformed, and sample IDs in iOS release contexts; the native Xcode release guard checks IDs captured at prebuild so archiving a development prebuild fails; release JavaScript validates embedded IDs at startup. Valid production IDs pass. There is no test-release bypass. The captain supplies `ADMOB_IOS_APP_ID` and `ADMOB_IOS_BANNER_ID` through the single `config/admob.js` identifier configuration. Catvertising is not linked into Android builds.
