...
test('Expo prebuild emits privacy configuration and a release guard that updates on regeneration', () => {
  ...
  try {
    ...
    const env = { ...process.env, CI: '1', NODE_ENV: 'development', EAS_BUILD_PROFILE: 'development', REVENUECAT_BUILD_MODE: '', EAS_BUILD_PLATFORM: 'ios' };
    ...
    assert.deepEqual(info.SKAdNetworkItems.map(item => item.SKAdNetworkIdentifier), ['cstr6suwn9.skadnetwork']);
